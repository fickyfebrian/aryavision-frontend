...
export const exportRecommendationToPDF = (
  ...
  const footerText =
    "Laporan ini dihasilkan secara otomatis oleh Sistem Rekomendasi Produk CCTV AryaVision menggunakan metode Content-Based Filtering untuk memberikan rekomendasi produk berdasarkan tingkat kemiripan karakteristik produk.";
  const splitFooter = doc.splitTextToSize(footerText, 180);
  doc.text(splitFooter, 14, finalY + 15);

  // 6. Simpan File (Penamaan otomatis)
  const safeName = selectedProduct.name
    .replace(/[^a-zA-Z0-9]/g, "-")
    .substring(0, 30);
